...
public:
    ...
    {
        qDebug() << "QQuickVtkItemPrivate constructor called";
    }
...
{
    qDebug() << "QQuickVtkItem constructor called";
...
class QSGVtkObjectNode : public QSGTextureProvider, public QSGSimpleTextureNode
{
    Q_OBJECT
public:
    QSGVtkObjectNode(vtkSmartPointer<vtkGenericOpenGLRenderWindow> window = nullptr) : vtkWindow(window)
    {
        qDebug() << "QSGVtkObjectNode()";
    ...
    {
        qDebug() << "Creating and initializing vtkWindow";

        // Create and initialize the vtkWindow if does not exist already
        if(!vtkWindow){
         qDebug() << "vtk window did not exist, so created a new one inside initialize method of QSGVtkObjectNode";
         vtkWindow = vtkSmartPointer<vtkGenericOpenGLRenderWindow>::New();
        } else{
            qDebug() << "using the existing vtkWindow, did not create a new one";
        }
...
QSGNode* QQuickVtkItem::updatePaintNode(QSGNode* node, UpdatePaintNodeData*)
{
    ...
    if (!n) {
        ...
        if (!d->node) {
            d->node = new QSGVtkObjectNode(m_WindowPtr);
        }
        n = d->node;
    }
